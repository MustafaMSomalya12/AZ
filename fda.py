...
COLUMNS_REQ = ['effective_time', 'spl_product_data_elements', 'drug_interactions',
                'openfda_manufacturer_name', 'openfda_brand_name', 'openfda_generic_name', 'openfda_route']

class FDA(object):
    def __init__(self, url=DOWNLOAD_URL):
        self.url = url
    def get_drug_label_files(self, nested_keys = ['results', 'drug', 'label', 'partitions']):
        try:
            ...
            print(err)
    def download_save_zip_files(self, files, path_to_save='./fda_data/drug_label/zip_files'):
        try:
            ...
            print(err)
    def unzip_files(self, files, path_to_save='./fda_data/drug_label/json_files'):
        try:
            if not os.path.exists(path_to_save):
                os.makedirs(path_to_save)
            self.json_files = []
            for f in files:
                ...
                with zipfile.ZipFile(f, 'r') as zip_obj:
                    ...
                    _ = [self.json_files.append(os.path.join(path_to_save, z)) for z in zip_obj.namelist()]
        except Exception as err:
            print(err)
    def merge_jsons_to_pandas_df(self, files, columns_req=None):
        try:
            ...
            merged_df = pd.concat([json_results_df, openfda_df], axis=1, sort=False)
    ...
    def save_pandas_df(self, pandas_df, path_to_save='./fda_data/drug_label/pandas', filename='pandas_df.csv'):
        try:
            if not os.path.exists(path_to_save):
                os.makedirs(path_to_save)
